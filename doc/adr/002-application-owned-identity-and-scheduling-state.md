...
**Reviewers:** @benjaminbradley (pending)
**Date:** 2026-09-14
...
store for campaign assets. Following from that, `doc/app/ARCHITECTURE.md`'s Data Ownership section
and `doc/contract/API.md` both currently assume `Campaign` (and, by omission, everything else) is a
KB concern.

`doc/contract/KB-PROJECTIONS.md` already carries a full ownership table splitting entities between
the KB and the application:
...
That document also flags the resulting conflict as unresolved: `doc/contract/API.md` states
`app-be` "does not own mutable canonical Character, Location, Campaign, Expedition, or similar
campaign-state records," which contradicts placing `Campaign` (and everything below it) on the
application side.

Separately, `doc/app/adr/004-role-based-permission-tiers.md` and
`doc/app/adr/005-email-and-password-authentication.md` assume `User` and `CampaignMembership` exist
...
- **KB-owned:** `Character`, `Region`, `Location`, `GMRegionAuthorization`, `ExpeditionReport`,
  `ProposedChange`.
- **Application-owned:** `User`, `Campaign`, `CampaignMembership`, `CharacterCommitment`,
  `AdventureOpportunity`, `GMAvailabilityWindow`, `CallToAdventure`, `ExpeditionParticipant`.

`KB-PROJECTIONS.md` remains the single source of truth for this table. This ADR does not duplicate
it going forward - it records why the split exists and resolves the one place it used to conflict
with another document.

### The KB stores references to app-owned entities as opaque facts, not owned relations

`KB-PROJECTIONS.md` already states the general rule: "Application records reference KB entity
identifiers. The reverse does not hold except where noted" - `Character.ownerUserId` is the one
noted exception today. The KB does not own, validate, or model the `User` record a `userId`
reference points at; referential integrity for these ids is an application concern.

---

## Consequences

- `doc/app/ARCHITECTURE.md`'s Data Ownership section is corrected to match this split rather than
...
- This is a joint-boundary decision. Status stays `Proposed` until @benjaminbradley confirms, even
  though the ownership table being ratified is his own.
...
- Whether the KB records application-computed availability at all, or only location/in-transit
  status - `doc/app/adr/003-downtime-commitments-and-availability.md` holds that availability is
...
- `doc/app/adr/004-role-based-permission-tiers.md`,
  `doc/app/adr/005-email-and-password-authentication.md` - the app-scoped decisions this makes
  concrete
- `doc/app/ARCHITECTURE.md` - Data Ownership section, updated alongside this ADR
