# ADR 001: Campaign State Changes Through Actions
...
Examples include ability scores, class and level, talents and abilities, spells, equipment, character description and notes, and other game-system-specific character-sheet values.
...
The application may translate this into a Travel activity with an origin and destination, start date, duration, blocking commitment, any required Shadowdark consequence roll, and a final location update when travel resolves successfully.

The same model applies to travel, downtime, crafting, training, recovery, joining an expedition, retirement, and other activities that alter shared campaign state.

### Availability remains derived

Availability is never directly edited by Players, GMs, or Administrators. It is derived from authoritative character state, including lifecycle status, location, commitments, expedition participation, and travel feasibility for the date/date range being evaluated.
...
| Current location | Request action | Override | Override | Primary mechanism |
...
- GM/Admin direct overrides should create audit history.
- Character availability must be recalculated from state rather than stored as a user-controlled value.
- Game-system-specific workflows can plug into the generic action model without changing the core character structure.
