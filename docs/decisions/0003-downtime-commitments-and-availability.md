# ADR 0003: Downtime Commitments and Availability
...
- `TRAINING`;
- `RECOVERY`;
- `OTHER` campaign-defined downtime types.
...
A commitment should include enough information to determine at least:
...
- end date;
- status;
- location or relevant destination when applicable;
- source action or record that created it;
- activity-specific data when needed.
...
The game-system or campaign-specific layer determines the mechanics of individual downtime types, such as:

- carousing cost and results;
- crafting inputs and completion effects;
- training requirements;
- travel consequences and location changes;
...
- The application will need a reliable mechanism to process date-based commitment transitions automatically.
